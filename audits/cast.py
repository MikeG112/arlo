...
class CAST(RiskLimitingAudit):
    ...
    def compute_initial_values(contests, margins, tolerance):
        '''
        Computes initial T and U values for setting up a CAST audit. 

        Input:
            contests  - dict of contest
            margins   - dict of margins
            tolerance -
        Output:
            vals     - dict of initial values, by contest:
                { 
                    contest1: {
                        'T'     : T,
                        'U'     : U,
    ...
    def get_expected_sample_sizes(self, margins, contests):
        """
        Returns the expected sample size for a CAST audit of each contest in contests.
